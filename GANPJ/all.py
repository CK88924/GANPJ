...
def train(dataset, epochs, output_dir):
    for epoch in range(epochs):
        ...
        for step, (input_image, target) in enumerate(dataset):
            ...
            # 检查是否有 NaN
            if np.isnan(gen_loss.numpy()).any() or np.isnan(disc_loss.numpy()).any():
...
# 使用保存的模型进行预测
input_image_path = r"dataset\test\example.png"
predicted_image = predict(input_image_path)

# 显示预测结果
plt.imshow((predicted_image[0] * 0.5 + 0.5).numpy())
plt.axis('off')
plt.show()
